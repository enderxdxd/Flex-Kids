        ...
        <div className="space-y-5">
          <Card padding="md" accent>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-7 h-7 rounded-lg bg-ink-100 text-ink-500 text-sm font-bold flex items-center justify-center shadow-brand-sm">1</span>
              <h2 className="text-caption uppercase text-ink-500">Responsável</h2>
            </div>
            <Input
              type="text"
            ...
            <select
              ...
              className="w-full h-10 px-3 text-sm bg-paper-raised border border-line rounded-lg hover:border-brand-300 focus:border-brand-500 focus-visible:shadow-focus focus:outline-none transition-all"
            >
            ...
            {selectedCustomer && (
              <div className="mt-3 p-3 bg-state-ok-soft border border-state-ok/30 rounded-lg">
                <p className="text-sm font-semibold text-state-ok">{selectedCustomer.name}</p>
                <p className="text-xs text-state-ok mt-0.5">
          ...
          <Card padding="md" className="bg-paper/60">
            <h3 className="font-semibold text-sm text-ink-800 mb-2">Informações</h3>
        ...
        <div className="lg:col-span-2">
          <Card padding="md" accent>
            <div className="flex items-center gap-2 mb-4">
              <span className="w-7 h-7 rounded-lg bg-ink-100 text-ink-500 text-sm font-bold flex items-center justify-center shadow-brand-sm">2</span>
              <h2 className="text-caption uppercase text-ink-500">Escolha o Pacote</h2>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {packageOptions.map((pkg) => (
                  <button
                    ...
                    className={cn(
                      'group text-left border border-line rounded-card-lg p-5 transition-all duration-200',
                      'bg-gradient-to-br from-white to-brand-50/30',
                      'hover:border-brand-400 hover:shadow-card-hover',
                    )}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-bold text-ink-900 group-hover:text-brand-700 transition-colors">{pkg.name}</h3>
                        <p className="text-xs text-ink-500 mt-0.5">{pkg.hours}h &middot; {pkg.expiryDays} dias</p>
                      </div>
                      <div className="w-9 h-9 bg-brand-gradient rounded-lg flex items-center justify-center text-white shadow-brand-sm transition-transform">
                        <PackageIcon size={18} />
                      </div>
                    </div>
                    <div className="flex items-end justify-between">
                      <div>
                        <p className="text-readout-sm text-ink-900 tabular-nums">{formatBRL(pkg.price)}</p>
                        <p className="text-[11px] text-ink-500">{formatBRL((pkg.price / pkg.hours))}/hora</p>
                      </div>
