  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-display text-ink-900">Configurações Fiscais</h1>
          <p className="text-sm text-ink-500 mt-1">Configure os dados da empresa e impressora fiscal Bematech</p>
        </div>
        <div className="flex gap-3">
          {config.enableFiscalPrint && (
            <button
              ...
              className="h-10 px-4 rounded-lg text-sm font-semibold bg-paper-raised text-brand-700 border border-brand-200 hover:bg-brand-50 hover:border-brand-300 transition-colors disabled:opacity-50 flex items-center gap-2"
            >
      ...
      <div className="bg-paper-raised rounded-card-lg shadow-card border border-line/80 p-6 space-y-6">
        <div className="bg-brand-50 border border-brand-200/50 p-4 rounded-lg">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={config.enableFiscalPrint}
              onChange={(e) => handleChange('enableFiscalPrint', e.target.checked)}
              className="w-5 h-5 accent-brand-600 rounded border-line focus-visible:outline-none focus-visible:shadow-focus"
            />
            <div>
              <span className="font-bold text-ink-900 flex items-center gap-1.5"><svg className="w-4 h-4 text-brand-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg> Emitir nota fiscal no check-out</span>
              <p className="text-xs text-ink-600">Imprime o cupom fiscal ao fechar cada visita</p>
            </div>
          </label>
        </div>

        <div>
          <h3 className="text-xl font-bold text-ink-900 mb-4 flex items-center gap-2"><svg className="w-5 h-5 text-ink-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg> Dados da empresa</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="w-full h-control px-3 text-sm bg-paper-raised border border-line rounded-md text-ink-900 placeholder:text-ink-400 focus:outline-none focus:border-brand-500 focus-visible:shadow-focus hover:border-line-strong transition-colors"
        ...
        <div>
          ...
          <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div className="border border-line rounded-card p-4">
              <p className="text-caption uppercase text-ink-500 mb-2">Antes de testar</p>
              <ul className="text-sm text-ink-600 space-y-1.5">
                <li><span className="font-medium text-ink-800">Driver:</span> instale o driver Bematech no Windows</li>
                <li><span className="font-medium text-ink-800">Conexão:</span> conecte a impressora via USB ou serial</li>
                <li><span className="font-medium text-ink-800">Detecção:</span> o sistema encontra a impressora sozinho</li>
              </ul>
            </div>

            <div className="border border-line rounded-card p-4">
              <p className="text-caption uppercase text-ink-500 mb-2">Ordem da configuração</p>
              <ol className="text-sm text-ink-600 space-y-1.5">
                <li><span className="font-mono text-xs text-ink-400 mr-1.5">1</span>Preencher os dados da empresa (CNPJ obrigatório)</li>
                <li><span className="font-mono text-xs text-ink-400 mr-1.5">2</span>Escolher o modelo da impressora</li>
                <li><span className="font-mono text-xs text-ink-400 mr-1.5">3</span>Deixar a porta em detecção automática</li>
                <li><span className="font-mono text-xs text-ink-400 mr-1.5">4</span>Marcar “Emitir nota fiscal no check-out”</li>
                <li><span className="font-mono text-xs text-ink-400 mr-1.5">5</span>Salvar e testar a impressora</li>
              </ol>
            </div>
          </div>
        </div>
