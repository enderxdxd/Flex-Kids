...
const UpdateChecker: React.FC = () => {
  ...
  if (!api) {
    return (
      <div className="bg-slate-800/50 rounded-card p-4 border border-slate-700/50">
        <h3 className="text-sm font-semibold text-ink-300 mb-2 flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.992 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182" /></svg> Atualizações</h3>
        <p className="text-xs text-ink-500">Disponível apenas na versão instalada.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-paper-raised/80 backdrop-blur-xl rounded-card-lg border border-white/20 shadow-md p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-caption uppercase text-ink-500">Atualizações</h3>
        </div>
        {appVersion && (
          <span className="text-[10px] font-bold bg-ink-100 text-ink-600 px-2.5 py-0.5 rounded-full">
            v{appVersion}
          </span>
        )}
      </div>
      ...
      {updateStatus.status === 'idle' && (
        <button
          onClick={handleCheck}
          className="w-full text-xs font-semibold bg-violet-600 hover:bg-violet-700 text-white py-2.5 px-4 rounded-card transition-all hover:shadow-md flex items-center justify-center gap-2"
        >
      ...
      {updateStatus.status === 'checking' && (
        <div className="flex items-center gap-2 text-sm text-ink-500 py-2">
          <svg className="w-4 h-4 animate-spin text-ink-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
          Verificando...
        </div>
      )}
      ...
      {updateStatus.status === 'up-to-date' && (
        <div className="space-y-2">
          <div className="bg-state-ok-soft rounded-card p-3 border border-state-ok/30">
            <p className="text-xs font-semibold text-state-ok flex items-center gap-1"><svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg> Você já está na versão mais recente!</p>
          </div>
          <button
            onClick={handleCheck}
            className="text-[11px] text-ink-400 hover:text-violet-600 transition-colors font-medium"
          >
            Verificar novamente
          </button>
        </div>
      ...
      {updateStatus.status === 'available' && (
        <div className="space-y-3">
          <div className="bg-state-warn-soft rounded-card p-3 border border-state-warn/30">
          ...
          <button
            ...
            className="w-full text-xs font-semibold bg-state-warn hover:bg-state-warn text-white py-2.5 px-4 rounded-card transition-all hover:shadow-md flex items-center justify-center gap-2"
          >
      ...
      {updateStatus.status === 'downloading' && (
        <div className="space-y-2">
          ...
          <div className="w-full bg-ink-200/60 rounded-full h-2">
            <div
      ...
      {updateStatus.status === 'downloaded' && (
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full text-xs font-semibold bg-state-ok hover:bg-[#166b4c] text-white py-2.5 px-4 rounded-card transition-all hover:shadow-md flex items-center justify-center gap-2"
          >
      ...
      {updateStatus.status === 'error' && (
        <div className="space-y-2">
          <div className="bg-state-bad-soft rounded-card p-3 border border-state-bad/30">
            <p className="text-xs font-semibold text-state-bad flex items-center gap-1"><svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg> Erro ao verificar</p>
            <p className="text-[11px] text-state-bad mt-0.5">{updateStatus.message}</p>
          </div>
          <button
            onClick={handleCheck}
            className="text-[11px] text-ink-400 hover:text-violet-600 transition-colors font-medium"
