...
const UpdateModal: React.FC<UpdateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-paper-raised/95 backdrop-blur-xl rounded-3xl shadow-card-lg border border-white/20 max-w-lg w-full animate-in zoom-in-95 duration-300">
        {/* Header */}
        <div className="p-6 border-b border-line/50">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gradient-to-br from-violet-100 to-purple-200 rounded-card-lg flex items-center justify-center">
        ...
        <div className="p-6 space-y-4">
          {/* Version Info */}
          <div className="flex items-center justify-between p-4 bg-gradient-to-br from-slate-50 to-slate-100/50 rounded-card-lg border border-line/50">
            <div>
              <p className="text-xs text-ink-500 font-semibold uppercase tracking-wide">Versão Atual</p>
              <p className="text-lg font-bold text-ink-700 mt-1">{currentVersion}</p>
            </div>
          ...
          {/* Downloaded */}
          {downloadState === 'downloaded' && (
            <div className="bg-state-ok-soft border border-state-ok/25 rounded-card-lg p-4">
              <p className="text-sm text-state-ok font-semibold">
          ...
          {releaseNotes && downloadState === 'idle' && (
            <div className="space-y-2">
              <h3 className="text-sm font-bold text-ink-700">Novidades:</h3>
              <div className="bg-paper/50 rounded-card-lg p-4 border border-line/50 max-h-48 overflow-y-auto">
          ...
          {/* Info */}
          {downloadState === 'idle' && (
            <div className="bg-blue-50/50 border border-blue-200/50 rounded-card-lg p-4">
        ...
        {/* Actions */}
        <div className="p-6 border-t border-line/50 flex gap-3">
          {downloadState === 'idle' && (
            <>
              <button
                onClick={onClose}
                className="flex-1 py-3 rounded-card border border-line-strong text-sm font-semibold text-ink-600 hover:bg-paper hover:border-slate-400 transition-all duration-200"
              >
                Mais Tarde
              </button>
              <button
                onClick={handleDownload}
                className="flex-1 py-3 rounded-card bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white text-sm font-bold transition-all duration-200 shadow-card hover:shadow-card-lg flex items-center justify-center gap-2"
              >
          ...
          {downloadState === 'downloading' && (
            <button
              disabled
              className="flex-1 py-3 rounded-card bg-ink-200 text-ink-500 text-sm font-semibold cursor-not-allowed"
            >
          ...
          {downloadState === 'downloaded' && (
            <button
              ...
              className="flex-1 py-3 rounded-card bg-state-ok hover:bg-[#166b4c] text-white text-sm font-bold transition-all duration-200 shadow-card hover:shadow-card-lg flex items-center justify-center gap-2"
            >
          ...
          {downloadState === 'error' && (
            <>
              <button
                onClick={onClose}
                className="flex-1 py-3 rounded-card border border-line-strong text-sm font-semibold text-ink-600 hover:bg-paper transition-all duration-200"
              >
                Fechar
              </button>
              <button
                onClick={handleDownload}
                className="flex-1 py-3 rounded-card bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white text-sm font-bold transition-all duration-200 shadow-card flex items-center justify-center gap-2"
              >
                Tentar Novamente
              </button>
            </>
